...
- **Keep the change scoped to one clear improvement, fix, or refactor.** A fix
  ...
  faster than the one.
- **Hana-Kami targets macOS, Windows and Linux.** Every change has to work
...
- **macOS, Windows, or Linux** — signed/notarized macOS builds, plus Windows and
  Linux builds, ship from the [releases page](https://github.com/TangerineSpecter/Hana-Kami/releases/latest).
...
- **A rewrite nobody asked for.** Large architectural changes need an issue or a
  [discussion](https://github.com/TangerineSpecter/Hana-Kami/discussions) with
