...
export class HiveManager {
   ...
   * agent cwd (never whole-disk), and namespaces every id `hanakami-<id>` so a server
  ...
  private injectedPrompt(
    ...
    const runtimeLine = rt
      ? `RUNNING BUILD: Hana-Kami v${rt.version}, ${rt.packaged ? 'packaged app' : 'local dev build'}${rt.appPath ? `, from ${rt.appPath}` : ''}. Say this version if asked which one is running, and do not assume behaviour from an older one. A local dev build inherits the launching shell's environment (umask included) where a packaged app does not, so file modes and inherited env can legitimately differ between the two. \`log.jsonl\` records an \`app-start\` event on every launch, which is how you spot a restart or a build switch.`
      : '';
   ...
   *  shim and merge a `hanakami-hive` hook group into agy's global hooks.json so a
  ...
  private installAgyHooks(): void {
    ...
    for (const p of [join(gem, 'config', 'hooks.json'), join(gem, 'antigravity-cli', 'hooks.json')]) {
      try {
        ...
        // The legacy group runs the same hooks; leave only one active bridge.
        delete existing['munder-hive'];
        existing['hanakami-hive'] = group;
    ...
    try {
      ...
      if (shim) {
        ...
        const command = process.platform === 'win32'
          ? this.nodeRunUnquoted(shim)
          : this.nodeRun(shim);
        config += '\n# --- hanakami-hive lifecycle hooks (auto-generated; do not edit) ---\n';
  ...
  private exposeCodexDataDir(
    ...
    const scanRoot = join(userHome, kind, 'hanakami');
    ...
    for (const entry of readdirSync(agents, { withFileTypes: true })) {
      ...
      for (const kind of ['sessions', 'archived_sessions'] as const) {
        ...
        try {
          ...
          const scanRoot = realpathSync(join(userHome, kind, 'hanakami'));
  ...
  private installPiHooks(dir: string): string {
    ...
    try {
      ...
      const manifest = { name: 'hanakami-hive-bridge', version: '0.3.1', main: 'extensions/hive-bridge.js', auto: true };
  ...
  private installGrokHooks(): void {
    ...
    try {
      ...
      writeFileSync(
        join(hookDir, 'hanakami-hive.json'),
        JSON.stringify({ hooks }, null, 2),
        'utf8'
      );
      const legacy = join(hookDir, 'munder-hive.json');
      const backup = `${legacy}.bak`;
      if (existsSync(legacy) && !existsSync(backup)) {
        try {
          const old = JSON.parse(readFileSync(legacy, 'utf8')) as { hooks?: unknown };
          if (old.hooks && JSON.stringify(old.hooks).includes('grok-hook.cjs')) {
            renameSync(legacy, backup);
          }
        } catch { /* preserve an unfamiliar hook file */ }
      }
