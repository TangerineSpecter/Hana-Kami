 ...
 * `hanakami://hire?src=<https-url>` deep link or an in-app file picker.
 *
...
export const HIRE_SPEC_V1 = 'hanakami/hire@1';
const LEGACY_HIRE_SPEC_V1 = 'munder-difflin/hire@1';
export const HIRE_PROTOCOLS = ['hanakami:', 'munderdifflin:'] as const;
...
export interface HireManifest {
  /** Spec tag; exactly `hanakami/hire@1` for this version. */
...
/** Parse a `hanakami://hire?src=<https-url>` deep link. Returns the https
...
export function parseHireDeepLink(link: string): string | null {
  ...
  if (!HIRE_PROTOCOLS.includes(u.protocol as (typeof HIRE_PROTOCOLS)[number])) return null;
  // Both hanakami://hire?src= (host) and hanakami:hire?src= (path).
