...
export const REPO = 'TangerineSpecter/Hana-Kami';
...
export function installerUrl(version: string, platform: string, arch: string): string {
  ...
  const file = platform === 'darwin' ? `Hana-Kami-${v}-mac-${arch}.dmg`
    : platform === 'win32' ? `Hana-Kami-${v}-win-x64-setup.exe`
    : `Hana-Kami-${v}-linux-x86_64.AppImage`;
  ...
  switch (status?.state) {
    ...
    case 'downloaded':
      return {
        ...
        detail: `Restart Hana-Kami to finish updating from v${v}.`,
...
export function manualInstallSteps(platform: string): { os: string; steps: string[] } {
  if (platform === 'darwin') {
    return {
      os: 'macOS',
      steps: [
        'Open the .dmg and drag Hana-Kami onto Applications. Choose Replace when asked.',
  ...
  if (platform === 'win32') {
    return {
      ...
      steps: [
        ...
        'Open Hana-Kami again and pick the same project.'
      ]
    };
  }
  return {
