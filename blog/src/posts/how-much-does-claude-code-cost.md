...
It multiplies, because every agent is its own Claude Code process with its own context window and its own token meter running in parallel, not a shared allowance split between them. Hana-Kami, the harness this blog belongs to, does not change what any of that costs: every agent it hires still runs on your own Claude Code subscription or API key, never a resold or marked up rate. What it adds is a lever on the multiplication rather than a discount on it: per agent token budgets cap what a single hired agent can spend, and a circuit breaker steers, then constrains, then stops an agent that loops, storms errors, or blows its budget, instead of a person finding out from the invoice ([Hana-Kami README](https://github.com/TangerineSpecter/Hana-Kami#readme), checked 10 Sep 2026).
