---
title: "Does the June 2026 Agent SDK Change Affect Hana-Kami?"
description: "From June 15, 2026 the Claude Agent SDK gets a separate credit. Hana-Kami drives the native Claude Code CLI, so your hive runs on your plan as before."
date: 2026-06-06
category: guides
categoryLabel: Guides
type: Technical
primaryKeyword: "claude agent sdk credit"
secondaryKeywords: ["hana-kami claude plan", "claude code subscription limits", "agent sdk vs claude code", "claude code june 15 2026"]
tags: ["Claude Code", "Local-First", "FAQ", "Use Cases"]
author:
  name: Chaitanya Giri
  initials: CG
faq:
  - q: "Does the June 15, 2026 Claude Agent SDK change affect Hana-Kami?"
    a: "The agents that do your work are unaffected. Hana-Kami runs its hive as interactive Claude Code terminal sessions, and Anthropic's update states that Claude Code in the terminal or IDE 'continues to use your subscription usage limits exactly as before.' The one exception is Hana-Kami's optional enrich assistant, which uses claude -p (print mode) and therefore draws on the new Agent SDK credit — but it's off by default and covered by that credit."
  ...
  - q: "Do I need to pay extra to keep running my Hana-Kami hive?"
...
before." Hana-Kami runs its hive as <strong>interactive Claude Code terminal sessions</strong> (via
...
If you run a Hana-Kami hive on a Claude Pro or Max plan, you may have seen Anthropic's June 15 Agent
...
mind; it's exactly where Hana-Kami sits on the safe side.
...
Claude Code CLI** the way you would by hand, it isn't. Hana-Kami is firmly the latter — it's a
[harness around Claude Code, not a wrapper around the SDK](/blog/claude-code-subagents-vs-multi-agent-harness/).

{% img "note-1" %}

## Why this doesn't change how Hana-Kami runs

Hana-Kami is a [multi-agent harness](/#what) that orchestrates the **native Claude Code CLI**. Each
...
We'd rather be precise than reassuring, so here's the caveat in full. Hana-Kami has **one** feature
...
You don't have to take our word for it. In Hana-Kami, every agent is a tab you can watch — a live
...
Hana-Kami drives the **Claude Code CLI** directly, so your [hive of agents](/blog/what-are-claude-code-agents/)
