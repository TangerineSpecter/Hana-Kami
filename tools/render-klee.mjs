...
const bundled = await build({entryPoints:['src/renderer/src/scene/office/kleeArt.ts'],bundle:true,write:false,format:'iife',globalName:'Klee'});
const sandbox = {}; runInNewContext(bundled.outputFiles[0].text,sandbox);
const {paintKlee,KLEE_W:W,KLEE_H:H} = sandbox.Klee;
class PixelContext {
...
for(const [row,d] of dirs.entries())for(const [group,a] of acts.entries())for(let frame=0;frame<4;frame++){
  const ctx=new PixelContext();paintKlee(ctx,d,a,frame);
  if(ctx.clipped)throw Error('Clipped drawing: '+[d,a,frame]);
  if(ctx.stack.length)throw Error('Unbalanced canvas state');
  blit(sheet,1024,ctx.data,(group*4+frame)*W,row*H,1);count++;
}
for(const [i,d] of dirs.entries()){const ctx=new PixelContext();paintKlee(ctx,d,'idle',0);blit(overview,1024,ctx.data,i*256,32,4);if(i===0){const front=Buffer.alloc(256*384*4);blit(front,256,ctx.data,0,0,4);await writeFile('outputs/klee-front.png',png(256,384,front));}}
await writeFile('outputs/klee-directions.png',png(1024,448,overview));
await writeFile('outputs/klee-spritesheet.png',png(1024,384,sheet));
