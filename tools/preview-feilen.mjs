...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>菲伦 · 动作预览</title>
<style>
...
</style><main><h1>菲伦 · 像素动作预览</h1><p>代码绘制 · 32 × 48 像素 · 四向造型<br>待机眨眼、交替迈步、打字抬手、阅读持书。场景中的桌椅独立绘制。</p><button id="pause">暂停动画</button>
<section><h2>坐姿桌沿遮挡 · 示意预览</h2><p>长发在桌沿处遮住，手和上身仍可出现在桌面上。此处桌子是示意，非楼层截图。</p><div class="grid" id="seats"></div></section>
...
dirs.forEach((d,row)=>acts.forEach((a,group)=>{for(let f=0;f<4;f++){Feilen.paintFeilen(cc,d,a,f);sheet.drawImage(cell,(group*4+f)*64,row*96,64,96);}}));
let paused=false,tick=0;document.getElementById('pause').onclick=e=>{paused=!paused;e.target.textContent=paused?'继续动画':'暂停动画'};
function draw(){items.forEach(({ctx,d,a})=>Feilen.paintFeilen(ctx,d,a,a==='idle'?(tick%16===15?3:0):tick%4));seats.forEach(({ctx,a})=>{ctx.clearRect(0,0,64,64);ctx.fillStyle='#79583e';ctx.fillRect(3,41,58,19);ctx.fillStyle='#c7a56c';ctx.fillRect(5,42,54,15);Feilen.paintFeilen(cc,'down',a,a==='idle'?(tick%16===15?3:0):tick%4,true);ctx.drawImage(cell,0,0,32,36,16,8,32,36);ctx.fillStyle='#515364';ctx.fillRect(23,43,20,12);ctx.fillStyle='#c4ccce';ctx.fillRect(25,45,16,7);ctx.fillStyle='#79583e';ctx.fillRect(3,57,58,3);});}draw();setInterval(()=>{if(!paused){tick++;draw()}},180);
</script></html>`;
await mkdir('outputs', { recursive: true });
await writeFile('outputs/feilen-preview.html', html);
console.log('outputs/feilen-preview.html');
