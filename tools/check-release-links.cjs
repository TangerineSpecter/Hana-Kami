 ...
 * `/releases/latest/download/Hana-Kami-<version>-<platform>.<ext>`. That URL
...
const assetRe = /Hana-Kami-(\d+\.\d+\.\d+)-([^\s`)]+)/g;
const assets = new Set();
for (const m of releaseMd.matchAll(assetRe)) {
  if (m[1] !== version) {
    problems.push(`RELEASE.md advertises Hana-Kami-${m[1]}-${m[2]} but package.json says ${version}`);
  }
  assets.add(`Hana-Kami-${m[1]}-${m[2]}`);
}
...
if (fs.existsSync(indexHtml)) {
  ...
  if (m && base) {
    for (const f of html.matchAll(/'Hana-Kami-' \+ REL \+ '([^']+)'/g)) {
      siteAssets.push(`${base[1]}Hana-Kami-${m[1]}${f[1]}`);
    }
  }
}
...
async function checkLive() {
  const base = 'https://github.com/TangerineSpecter/Hana-Kami/releases/latest/download/';
