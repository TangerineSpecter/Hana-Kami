...
const bundled = await build({entryPoints:['src/renderer/src/scene/office/anyaArt.ts'],bundle:true,write:false,format:'iife',globalName:'Anya'});
const sandbox = {}; runInNewContext(bundled.outputFiles[0].text,sandbox);
const {paintAnya,ANYA_W:W,ANYA_H:H} = sandbox.Anya;
class PixelContext {
...
for(const [row,d] of dirs.entries())for(const [group,a] of acts.entries())for(let frame=0;frame<4;frame++){
  const ctx=new PixelContext();paintAnya(ctx,d,a,frame);
...
for(const [i,d] of dirs.entries()){const ctx=new PixelContext();paintAnya(ctx,d,'idle',0);blit(overview,1024,ctx.data,i*256,32,8);if(i===0){const front=Buffer.alloc(256*384*4);blit(front,256,ctx.data,0,0,8);await writeFile('outputs/anya-front.png',png(256,384,front));}}
await writeFile('outputs/anya-directions.png',png(1024,448,overview));
await writeFile('outputs/anya-spritesheet.png',png(1024,384,sheet));
console.log('Rendered '+count+' frames. Integer pixels, no clipped drawing, balanced canvas state.');

// Same pixel grid and scale: Feilen on the left, Anya on the right.
...
const yc=new PixelContext();paintAnya(yc,'down','idle',0);
blit(comparison,576,fc.data,16,16,8);blit(comparison,576,yc.data,304,16,8);
await writeFile('outputs/anya-feilen-comparison.png',png(576,416,comparison));
