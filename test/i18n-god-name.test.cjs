...
//
// These tests hold both fixes, plus the rule that a saved language choice always
// wins over the fresh-install default.
...
test('with nothing saved the app starts in Simplified Chinese, never the OS locale', () => {
  ...
  assert.match(body, /return 'zh-CN';/, 'detectLanguage does not fall back to Simplified Chinese');
});
